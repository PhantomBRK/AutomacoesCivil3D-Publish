...
- **Corredores automáticos:** `PLANVIAS_CRIAR_CORREDORES` cria, para cada via, alinhamento sem site a partir do eixo (mantendo a polyline), perfil de greide por PVIs amostrados a cada 20 m sobre a superfície escolhida (ou plano na cota 0), assembly montada da seção-tipo com subassemblies de catálogo (BasicLane/BasicCurbAndGutter/BasicSideWalk; lado direito montado do eixo para fora e espelhado) — reutilizando assembly existente com o nome da seção, se houver — e o corredor com baseline e regiões que param nas caixas das junções (folga = meia-caixa da via cruzada + raio de meio-fio). Nomes usam o identificador da via (`AL_VIA_xxxx`, `PF_`, `COR_`); rodar de novo pula vias já processadas.
- **Corredores das interseções:** cada arco de retorno de meio-fio (anel da borda de pavimento) vira uma baseline no corredor único `COR_JUNCOES_PLANVIAS`, com alinhamento invertido (interior do cruzamento no lado direito), perfil de 2 PVIs com cotas amarradas aos greides das vias nos pontos de tangência (cota do eixo − caimento de 2% até a borda; fallback superfície → cota 0) e assembly de pista interna com largura até o cruzamento dos eixos (reutilizada em degraus de 0,5 m). Reexecutar pula retornos já processados (baseline por handle do arco).
