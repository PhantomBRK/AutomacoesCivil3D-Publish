...
- **Corredores automáticos:** `PLANVIAS_CRIAR_CORREDORES` cria, para cada via, alinhamento sem site a partir do eixo (mantendo a polyline), perfil de greide por PVIs amostrados a cada 20 m sobre a superfície escolhida (ou plano na cota 0) e o corredor com baseline e regiões que param nas caixas das junções (folga = meia-caixa da via cruzada + raio de meio-fio). **A assembly de cada seção-tipo vem do conjunto definido pelo usuário** (`PLANVIAS_DEFINIR_ASSEMBLIES` ou seleção na tela na primeira execução, memorizada em `%AppData%\AutomacoesCivil3D\PlanejadorVias\Assemblies.json`), com fallbacks: assembly homônima à seção no desenho → montagem automática com o catálogo stock (apenas se escolhida via keyword "Automatica") → sem assembly, o corredor da via não é criado (alinhamento/perfil sim, com aviso). Nomes usam o identificador da via (`AL_VIA_xxxx`, `PF_`, `COR_`); rodar de novo pula vias já processadas.
