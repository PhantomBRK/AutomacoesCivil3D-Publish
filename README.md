...
| PLANVIAS_CRIAR_CORREDORES | Gera a cadeia Civil 3D completa por via — Alinhamento → Perfil → Assembly → Corredor — com greide colado a uma superfície (ou plano), regiões paradas nas caixas das junções e, em seguida, os corredores das interseções |
| PLANVIAS_CORREDORES_JUNCOES | (Re)gera só os corredores das interseções: baselines nos retornos de meio-fio com perfis amarrados aos greides das vias |
| PLANVIAS_DEFINIR_ASSEMBLIES | Define o conjunto de assemblies dos corredores: uma por seção-tipo + a das interseções (seleção na tela, memorizada em %AppData%) |
| PLANVIAS_REGERAR_GREIDES | Regera os greides das vias já processadas sem recriar nada: limpa os PVIs dos perfis existentes, reaplica o greide novo e reconstrói os corredores |
...
- **Corredores automáticos:** `PLANVIAS_CRIAR_CORREDORES` cria, para cada via, alinhamento sem site a partir do eixo (mantendo a polyline), **perfil de greide com curvas verticais** — PVIs a cada 20 m sobre a superfície com **parábolas simétricas** (até 40 m, limitadas pela distância entre PVIs), **cota suavizada** por média móvel (±15 m) e **PVI de cota única em cada cruzamento** (as duas vias se encontram exatamente na mesma altura na junção, sem degrau) — e o corredor com baseline e regiões que param nas caixas das junções (folga = meia-caixa da via cruzada + raio de meio-fio). **A assembly de cada seção-tipo vem do conjunto definido pelo usuário** (`PLANVIAS_DEFINIR_ASSEMBLIES` ou seleção na tela na primeira execução, memorizada em `%AppData%\AutomacoesCivil3D\PlanejadorVias\Assemblies.json`), com fallbacks: assembly homônima à seção no desenho → montagem automática com o catálogo stock (apenas se escolhida via keyword "Automatica") → sem assembly, o corredor da via não é criado (alinhamento/perfil sim, com aviso). Nomes usam o identificador da via (`AL_VIA_xxxx`, `PF_`, `COR_`); rodar de novo pula vias já processadas.
- **Corredores das interseções:** cada arco de retorno de meio-fio (anel da borda de pavimento) vira uma baseline no corredor único `COR_JUNCOES_PLANVIAS`, com alinhamento invertido (interior do cruzamento no lado direito, que é o lado padrão das subassemblies) e perfil de 2 PVIs com cotas amarradas aos greides das vias nos pontos de tangência (cota do eixo − caimento de 2% até a borda; fallback superfície). **Usa a assembly de interseções definida pelo usuário** (recomenda-se uma pista larga no lado direito — ela preenche do meio-fio até o centro); sem assembly definida, ou sem greide/superfície para amarrar as cotas, o retorno é **pulado com aviso** (nada de interseção "solta" na cota 0). Reexecutar pula retornos já processados (baseline por handle do arco).
- **Limitações da versão inicial:** os trechos são assumidos localmente retos no entorno imediato do cruzamento (caso típico de malha urbana); os quantitativos não descontam as caixas das junções; os eixos permanecem contínuos através das junções; a montagem automática de assemblies depende do catálogo stock do Civil 3D (quando indisponível, a assembly é criada e o usuário completa pela paleta); nas interseções, meio-fio e calçada ao longo dos retornos ainda não são modelados no corredor (a pista interna sim) e as pistas internas dos quatro cantos se sobrepõem no miolo — adequado a nível de planejamento.
