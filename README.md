...
| PLANVIAS_CRIAR_CORREDORES | Gera a cadeia Civil 3D completa por via — Alinhamento → Perfil → Assembly → Corredor — com greide colado a uma superfície (ou plano) e regiões do corredor paradas nas caixas das junções |
...
- **Limitações da versão inicial:** os trechos são assumidos localmente retos no entorno imediato do cruzamento (caso típico de malha urbana); os quantitativos não descontam as caixas das junções; os eixos permanecem contínuos através das junções; a montagem automática de assemblies depende do catálogo stock do Civil 3D (quando indisponível, a assembly é criada e o usuário completa pela paleta); interseções modeladas (corridor intersections) ficam para uma etapa futura — as regiões param nas junções.
